...
pushd /home/ec2-user/spark-ec2 > /dev/null
...
echo "RSYNC'ing /home/ec2-user/spark-ec2 to other cluster nodes..."
rsync_start_time="$(date +'%s')"
for node in $SLAVES $OTHER_MASTERS; do
  echo $node
  rsync -e "ssh $SSH_OPTS" -az /home/ec2-user/spark-ec2 $node:/home/ec2-user &
  scp $SSH_OPTS ~/.ssh/id_rsa $node:.ssh &
  sleep 0.1
done
wait
rsync_end_time="$(date +'%s')"
echo_time_diff "rsync /home/ec2-user/spark-ec2" "$rsync_start_time" "$rsync_end_time"
...
for module in $MODULES; do
  ...
  cd /home/ec2-user/spark-ec2  # guard against init.sh changing the cwd
done
...
chmod u+x /home/ec2-user/spark/conf/spark-env.sh
...
for module in $MODULES; do
  ...
  cd /home/ec2-user/spark-ec2  # guard against setup.sh changing the cwd
done
